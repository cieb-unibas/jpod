...
sqlite3 $jpod_version ".read ${jpod_test_path}jpod/scripts/jpod_create.sqlite"
ml purge
...
cd ./jpod/
python scripts/test_db.py $jpod_test_path
...
do
  python scripts/$f $jpod_test_path $jpod_version
done